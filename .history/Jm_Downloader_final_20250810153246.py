...
from PySide6.QtGui import (QPixmap, QPalette, QBrush, QFont, QIcon, 
                           QPainter, QColor, QRadialGradient, QLinearGradient ,QPen ,QCursor)
...
class ParticleWidget(QWidget):
    def __init__(self, parent=None, particle_count=90):
        ...
        self.setAttribute(Qt.WA_TransparentForMouseEvents, False)  # 允许点击事件
        ...
        self.mouse_influence = 120
        self.mouse_strength = 0.6
        self.mouse_mode = "attract"  # 可选 "repel"

        self.click_waves = []  # 记录波纹效果
    ...
    def resizeEvent(self, event):
        ...
        if not self.particles:
            ...
            for p in self.particles:
                p.pos.setX(min(max(0, p.pos.x()), w))
                p.pos.setY(min(max(0, p.pos.y()), h))
        super().resizeEvent(event)

    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            # 点击时添加波纹
            self.click_waves.append({
    ...
    def on_timer(self):
        ...
        for p in self.particles:
            ...
            if dist < self.mouse_influence:
                nx = dx / dist
                ny = dy / dist
                force = (1 - dist / self.mouse_influence) * self.mouse_strength
                if self.mouse_mode == "repel":
                    p.vel.setX(p.vel.x() - nx * force)
                    p.vel.setY(p.vel.y() - ny * force)
                else:
                    p.vel.setX(p.vel.x() + nx * force)
                    p.vel.setY(p.vel.y() + ny * force)
            p.vel *= 0.995
        ...
        # 波纹扩散更新
        for wave in self.click_waves:
            wave["radius"] += 3
            wave["alpha"] = max(0, wave["alpha"] - 4)

        # 移除透明度为 0 的波纹
        self.click_waves = [w for w in self.click_waves if w["alpha"] > 0]
    ...
    def paintEvent(self, event):
        ...
        for i in range(len(self.particles)):
            ...
            for j in range(i + 1, len(self.particles)):
                ...
                dy = a.pos.y() - b.pos.y()
                d2 = dx * dx + dy * dy
                if d2 <= self.max_link_distance ** 2:
                    d = math.sqrt(d2)
                    alpha = int(200 * (1 - d / self.max_link_distance))
                    if alpha <= 0:
                        continue
                    pen.setColor(QColor(200, 200, 255, alpha))
                    painter.setPen(pen)
                    painter.drawLine(a.pos, b.pos)

        # 点击波纹
        for wave in self.click_waves:
            pen.setColor(QColor(255, 255, 255, wave["alpha"]))
            pen.setWidth(2)
            painter.setPen(pen)
            painter.setBrush(Qt.NoBrush)
            painter.drawEllipse(wave["pos"], wave["radius"], wave["radius"])
...
# 资源路径
BACKGROUND_IMAGE_PATH = "EVA.png"  # 背景图片路径
...
class JmcomicDownloader(QMainWindow):
    ...
    def load_config(self):
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                ...
                if 'Settings' in self.config and 'download_dir' in self.config['Settings']:
    ...
    def init_ui(self):
        # 粒子背景层
        self.particle_widget = ParticleWidget(self, particle_count=90)
    ...
    def set_background_image(self):
        ...
        try:
            if os.path.exists(BACKGROUND_IMAGE_PATH):
                ...
                if pixmap.isNull():
                    self.status_text.append("⚠️ 背景图片加载失败，格式可能不支持")
                    return
                ...
                self.status_text.append(f"⚠️ 背景图片未找到: '{BACKGROUND_IMAGE_PATH}'")
        except Exception as e:
            self.status_text.append(f"❌ 设置背景时出错: {str(e)}")
            self.logger.error(f"设置背景时出错: {str(e)}")
            self.setStyleSheet("background-color: #2c3e50;")
...
if __name__ == "__main__":
    ...
    # 获取资源路径
    BACKGROUND_IMAGE_PATH = resource_path("EVA.png")
